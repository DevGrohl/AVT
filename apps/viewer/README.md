...
- **Code flow paths** — experimental selected-Entry-Point view that uses a compact top-down hierarchy: the Entry Point/root starts near the top, normal calls descend through the center/right, External Interactions attach above their caller, and triggered Error/Success outcomes branch below with errors biased left. When the analyzer provides a safe trigger condition, `raise` and `return` markers create viewer-only outcome nodes such as `Error when not account` or `Error when except ValueError`.
