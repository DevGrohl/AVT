...
- **Code flow paths** — experimental selected-Entry-Point view that lays behavior left-to-right by call depth and separates branch/loop/outcome/external lanes using Flow Marker evidence so a Developer can see possible paths and why they exist. When the analyzer provides a safe trigger condition, `raise` and `return` markers create viewer-only Error/Success outcome nodes such as `Error when not account` or `Error when except ValueError`.
...
### Analysis Overlay editing

Click an uncertain edge to inspect it. The edge inspector can mark uncertain edges as confirmed or rejected in an in-memory Analysis Overlay. Use **Export overlay JSON** to download a file compatible with analyzer `--overlay` / `<project>/.avt/overlay.json`.

Overlay edits are presentation/session state until exported. They do not modify source code.
