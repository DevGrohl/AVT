...
- **Code flow paths** — experimental selected-Entry-Point view that uses a NotebookLM-like mindmap shape: the Entry Point/root starts on the left, normal calls branch to the upper-right, External Interactions attach above their caller, and triggered Error/Success outcomes branch below. When the analyzer provides a safe trigger condition, `raise` and `return` markers create viewer-only outcome nodes such as `Error when not account` or `Error when except ValueError`.
